...
def detect(
    ...
    (save_dir / 'metric').mkdir(parents=True, exist_ok=True)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    iou, date_diff, close_diff, T = stockimg.TFPN(dates, cls)
                    score_df = pd.DataFrame({
                        'Label': [cls.item()],
                        'IoU': [iou],
                        'DateD': [date_diff],  # Labeling - Detection
                        'CloseD': [close_diff],
    ...
    save_path = str(save_dir / 'metric' / 'score.csv')
